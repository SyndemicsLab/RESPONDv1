...
# initial cohort and background mortality are deterministic, so they are not generated within this function.

generate_inputs <- function (){
  source("codes/calibration/utility_functions.R")
  source("codes/calibration/prior_draw.R")
  ...
  # ------------------------------------------------------------------------------------------------------------------
  # entering cohort
  entering_cohort_proportions <- generate_entering_cohort()
  entering_cohort_matrix <<- entering_cohort_proportions
  for (i in 1:length(entering_cohort_total_size))
  {
    entering_cohort_matrix[,i] <<- entering_cohort_proportions[,i]*entering_cohort_total_size[i]
  }
  ...
  if (save_input_files_as_csv == "yes")
  {
    ...
    col_names <- c(paste("proportion_of_new_comers_c",time_varying_entering_cohort_cycles[1],sep=""))
    for (i in 2:(length(time_varying_entering_cohort_cycles)))
    {
      col_names <- c(col_names,paste("proportion_of_new_comers_c",time_varying_entering_cohort_cycles[i],sep=""))
    }
    entering_cohort_tbl <- data.frame(factor_perm,entering_cohort_proportions)
  ...
  #------------------------------------------------------------------------------------------------------------------
  # all types overdose
  all_types_overdose_matrix <<- generate_all_types_overodse()
  ...
  if (save_input_files_as_csv == "yes")
  {
    ...
    colnames(factor_perm)<-c("oud","sex","agegrp","block")
    factor_perm <- factor_perm[,c("block","agegrp","sex","oud")]
    col_names <- c("block","agegrp","sex","oud")
  ...
  fatal_overdose_vec <<- generate_fatal_overdose()
  fatal_overdose_matrix <- matrix(fatal_overdose_vec,nrow = 1)
  
  if (save_input_files_as_csv == "yes")
  {
    col_names <- rep("",length(time_varying_overdose_cycles))
    for (i in 1:(length(col_names)))
    {
      col_names[i] <- paste("fatal_to_all_types_overdose_ratio_c",time_varying_overdose_cycles[i],sep="")
    }
