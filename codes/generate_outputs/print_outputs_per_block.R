...
print_outputs_per_block <- function() {
  ...
  for (i in 1:imax)
  {
    general_output_name <- paste("output",strategy_id,"/counts_block",i-1,".csv", sep='')
    mortality_output_name <- paste("output",strategy_id,"/bg_mortality_block",i-1,".csv", sep='')
  ...
  for (i in 1:imax)
  {
    overdose_output_name <- paste("output",strategy_id,"/all_overdose_block",i-1,".csv", sep='')
    first_column <- num_comp_per_block*(i-1)+1
    last_column <- first_column + num_comp_per_block -1
